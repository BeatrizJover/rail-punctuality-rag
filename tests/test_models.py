...
def test_staging_mirrors_the_fact_measures() -> None:
    """A column added to the fact but not to staging would break the loader."""
    ignored = {"loaded_at"}
    assert set(stg_fact_stop_event.c.keys()) == set(fact_stop_event.c.keys()) - ignored
